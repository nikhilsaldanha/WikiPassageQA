...
def thread_function(i, till_len):
    ...
    passages_df.to_csv(PROCESSED_DATA_DIR+'/passage_df_WebAP_'+str(i)+'_.csv', index=False)
...
if __name__ == "__main__":
    ...
    RAW_DATA_DIR = os.path.join(DATA_DIR, "extracted/")
    PROCESSED_DATA_DIR = os.path.join(DATA_DIR, "processed")

    passages_path = RAW_DATA_DIR+ "/webap_passages.json"
    ...
    with open(passages_path, 'r') as f:
        ...
        for i in range(17):
            df = pd.read_csv(PROCESSED_DATA_DIR+'/passage_df_WebAP_'+str(i)+'_.csv')
            print(df.shape)
            passages_df = passages_df.append(df)
            os.remove(PROCESSED_DATA_DIR+'/passage_df_WebAP_'+str(i)+'_.csv')

        print(passages_df.shape)
        passages_df.to_csv(PROCESSED_DATA_DIR+'/passage_df_WebAP.csv', index=False)
